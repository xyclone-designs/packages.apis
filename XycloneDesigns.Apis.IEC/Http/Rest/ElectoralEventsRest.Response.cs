...
namespace XycloneDesigns.Apis.IEC.Http.Rest
{
	...
	{
		public class Response : ElectoralEvents.Response
		{
			public Response() { }
			public Response(Request request, IEnumerable<ElectoralEvent> items)
			{
				IEnumerable<ElectoralEvent> enumerable = GeneralRest.Filter(items, (item, filter) => filter.Filter(item), request.Filter);
