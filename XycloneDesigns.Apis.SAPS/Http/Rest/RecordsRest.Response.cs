...
namespace XycloneDesigns.Apis.SAPS.Http.Rest
{
	...
	{
		public class Response : Records.Response
		{
			public Response() { }
			public Response(Request request, IEnumerable<Record> items) 
			{
				IEnumerable<Record> enumerable = GeneralRest.Filter(items, (item, filter) => filter.Filter(item), request.Filter);
