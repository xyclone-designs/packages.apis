...
namespace XycloneDesigns.Apis.SAPS.Http.Rest
{
	...
	{
		public class Response : PoliceStations.Response
		{
			public Response() { }
			public Response(Request request, IEnumerable<PoliceStation> items)
			{
				IEnumerable<PoliceStation> enumerable = GeneralRest.Filter(items, (item, filter) => filter.Filter(item), request.Filter);
